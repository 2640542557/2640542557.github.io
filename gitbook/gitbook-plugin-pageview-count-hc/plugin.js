...
function requestCount(targetUrl) {
  // return $.ajax('http://127.0.0.1:8029/yqsh/bbs/common/count',{
  //   data:{url:encodeURIComponent(targetUrl)},
  // })
  var param = {url: targetUrl}
  return $.ajax({
	  type: "post",
	  url: "/yqsh/bbs/common/count",
...
require(["gitbook", "jQuery"], function (gitbook, $) {
  ...
  function resetViewCount() {
    ...
    var renderWrapper = $('<div class="page-view-wrapper dropdown pull-left">\
        ...
        <span class="page-view-counter" title="阅读数">5</span>\
      </div>')
    ...
    requestCount(location.href).then(function(data){
	  console.log('阅读次数:'+JSON.stringify(data))
